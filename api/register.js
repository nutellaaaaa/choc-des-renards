...
module.exports = async function handler(req, res) {
  ...
  const {
    ...
    acknowledged, // case à cocher obligatoire
  } = req.body || {}
  ...
  try {
    ...
    try {
      await resend.emails.send({
        ...
        html: `<h2>Nouvelle demande d'inscription</h2><p><strong>Pseudo :</strong> ${username}</p><p><strong>Nom :</strong> ${lastName}</p><p><strong>Prénom :</strong> ${firstName}</p><p><strong>Téléphone :</strong> ${phone}</p><p><strong>Catégorie :</strong> ${category}</p><br><p>⏳ En attente de validation dans le panneau d'administration.</p>`,
      })
